...
def discretize_edges(edges, num_points=100):
    """Discretize multiple edges into a list of points (num_points total)."""
    all_pts = []
    for edge in edges:
        all_pts += discretize_edge(edge, max(2, num_points // max(1, len(edges))))
    ...
    pts_arr = np.array([np.array(p.Coord()) for p in all_pts])
    idx = np.linspace(0, len(pts_arr)-1, num_points).astype(int)
    return [gp_Pnt(*pts_arr[i].tolist()) for i in idx]
...
# #
# # sys.path.append("/mnt/sdb/TMEMJ/CADDreamer/pyransac/cmake-build-release")
# # import fitpoints
...
def cone_to_pyocc(cone, height=2):
    ...
    cone_axis = gp_Ax2(cone_pos, cone_dir)
    # OCC rejects a cone whose two radii are identical (degenerate → DomainError).
    # When the fitted semi-angle is ~0 the top radius collapses to 0 == bottom radius,
    # so clamp it to a small non-zero value to keep a valid cone.
    r_top = np.abs(np.tan(cone.m_angle) * height)
    if r_top < 1e-4:
        r_top = 1e-3
    cone_shape = BRepPrimAPI_MakeCone(cone_axis,
                                        0,
...
def getIntersecVertices(cut_res, newton_shapes, primitive_idxes):
    ...
    rever_vertices_arrs = []
    explorer = TopExp_Explorer(cut_res, TopAbs_VERTEX)
    # Guard against primitive indices outside newton_shapes (stale/merged labels).
    candidate_shapes = [newton_shapes[int(idx)] for idx in primitive_idxes
                        if 0 <= int(idx) < len(newton_shapes)]
    while explorer.More():
        current_v = topods.Vertex(explorer.Current())
        current_point = BRep_Tool.Pnt(current_v)
        p_arr = np.array([current_point.X(), current_point.Y(), current_point.Z() ])
...
def getIntersecEdges(cut_result, shapes, newton_shapes, current_index, startnode_primitives, endnode_primitives, start_vertex, end_vertex, coordinates):
    ...
    edge_primitives = list(startnode_primitives.intersection(endnode_primitives))
    # Need two in-range primitive indices to test edge membership; otherwise no edges qualify.
    _ep_valid = (len(edge_primitives) >= 2
                 and 0 <= int(edge_primitives[0]) < len(newton_shapes)
                 and 0 <= int(edge_primitives[1]) < len(newton_shapes))
    all_edges = []
    explorer = TopExp_Explorer(cut_result, TopAbs_EDGE)
    while explorer.More():
        current_edge= topods.Edge(explorer.Current())
        if _ep_valid and edge_on_face(current_edge, newton_shapes[int(edge_primitives[0])]) and edge_on_face(current_edge, newton_shapes[int(edge_primitives[1])]):
            vertices = getVertex(current_edge)
            if start_vertex is None or end_vertex is None:
                if current_edge.Orientation() == 0:
                    all_edges.append(current_edge)
...
def get_select_edges(shapes, newton_shapes,  all_loops):
    ...
    for loops_idx in range(len(all_loops)):
        ...
        for current_idx in range(len(loops)):
            ...
            for startnode_primitives, edge_primitives, endnode_primitives, (ss_coord, ee_coord, coordinates, loop_node_idx ) in loop:
                edge_primitives = sorted([int(iii) for iii in edge_primitives])
                # Skip loops referencing primitive indices outside the shapes list
                # (stale/merged labels on a noisy mesh) — avoids IndexError downstream.
                ...
                select_edges_0, select_edges_1, removed_edges_0, removed_edges_1, edge_map, edge_to_vertices_map = get_select_intersectionline(shapes,
                                                                                            ...
                                                                                            edge_primitives,
                                                                                            coordinates, 0)
                # This primitive pair produced no intersection edge — skip it.
                if select_edges_0 is None or select_edges_1 is None:
                    continue
                current_face_idx = loops_idx
                other_face_idx = [p_idx for p_idx in edge_primitives if p_idx != current_face_idx][0]
                if other_face_idx not in primitive_intersection[current_face_idx].keys():
...
def get_mesh_patch_boundary_face(mesh, comp, facelabel):
    ...
    # No large boundary cycle found for this patch (e.g. closed/manifold segment):
    # there are no boundary edges to trace, so this patch contributes no loops.
    ...
    while len(real_edges_comp) != 0:
        ...
        if  len(c_comp_start) == 1 and len(c_comp_end) == 1:
            ...
            # Topology should have 3 primitives meeting at a corner node and 2 along an edge.
            # A noisy mesh can violate this; skip appending the malformed loop but still
            # advance the traversal below (avoids crashing / infinite loop).
            _topology_ok = (len(node_to_start_primitives) == 3 and len(end_to_node_primitives) == 3
                            and len(edge_primitives) == 2)
...
def get_edge_pairs(edges1, edges2, coordinates):
    ...
    # No shared intersection point between the two edge sets: fall back to the first
    # edge of each (avoids argmin on an empty sequence). Caller indexes these directly.
    if len(distance_to_path) == 0:
        return 0, 0
    choose_edge_pair = np.argmin(distance_to_path)
    s_idx1, s_idx2 = all_candidates_pairs[choose_edge_pair]
    return s_idx1, s_idx2
...
def remove_abundant_edges(edges, primitive):
    ...
    edges_label = [ee.Orientation() for ee in edges]
    if len(edges_label) == 0:
        # No edges to process for this primitive pair: return empty result.
        return [], dict(), dict()
    edges_label_choose = min(set(edges_label))
    edges = [ee for ee in edges if ee.Orientation() == edges_label_choose]
...
def get_vertex(shapes, newton_shapes,  current_index, startnode_primitives):
    current_face = shapes[current_index]
    # Guard against primitive indices that fall outside the shapes list (stale/merged labels).
    other_faces = [shapes[int(index)] for index in startnode_primitives
                   if index != current_index and 0 <= int(index) < len(shapes)]
    # other_faces = [large_shapes[int(index)] for index in startnode_primitives if index != current_index]
    # other_rep = Compound(other_faces)
    current_shape = current_face
    for face in other_faces:
...
def get_edge(shapes,  newton_shapes, current_index, startnode_primitives, endnode_primitives, start_vertex, end_vertex, coordinates):
    primitives = startnode_primitives.union( endnode_primitives)
    current_face = shapes[current_index]
    other_faces = [shapes[int(index)] for index in primitives
                   if index != current_index and 0 <= int(index) < len(shapes)]
    other_rep = Compound(other_faces)
    cut_result = BRepAlgoAPI_Cut(current_face, other_rep).Shape()
    start_l_tuple, end_l_tuple, paths = getIntersecEdges(cut_result, shapes, newton_shapes, current_index,
...
def get_select_intersectionline(shapes, newton_shapes, edge_primitives, coordinates, coordinates_assign):
    ...
    # No intersection edges between this primitive pair (degenerate cut on noisy mesh):
    # signal the caller to skip this pair instead of indexing into an empty list.
    if len(new_edges_0) == 0 or len(new_edges_1) == 0:
        _t0 = newton_shapes[int(edge_primitives[0])].getType() if int(edge_primitives[0]) < len(newton_shapes) else '?'
        _t1 = newton_shapes[int(edge_primitives[1])].getType() if int(edge_primitives[1]) < len(newton_shapes) else '?'
        print(f"[GUARD] pair {edge_primitives} ({_t0},{_t1}): cut0={len(cut_edges_0)} cut1={len(cut_edges_1)} orig0={len(original_edges_0)} new0={len(new_edges_0)} new1={len(new_edges_1)}")
...
def get_loop_face(shapes, newton_shapes, loop_index, loops, new_trimesh,
    ...
    for loop in loops:
        ...
        for startnode_primitives, edge_primitives, endnode_primitives, (ss_coord, ee_coord, coordinates, loop_node_idx ) in loop:
            ...
            # --- Robustness guard: intersection edges for some primitive pairs may be
            # missing (skipped earlier on noisy meshes). Skip loop entries that
            # reference them instead of raising KeyError during face assembly. ---
            try:
                _sel = select_edges[loop_index]
            except Exception:
                continue
            def _keys_ok(idx_set, prefix, node_idx):
                if len(idx_set) == 0:
                    return False
                _d = _sel.get(int(list(idx_set)[0])) if hasattr(_sel, 'get') else None
                return _d is not None and any(k.startswith(prefix + str(node_idx)) for k in _d.keys())
            if not _keys_ok(edge_primitives.difference(set([loop_index])), 'start_', loop_node_idx[0]):
                print(f"[GUARD] loop entry skipped: edge dict missing for pair ({loop_index}, {edge_primitives})")
                continue
            if len(startnode_primitives.difference(edge_primitives)) != 0:
                # else-branch below also needs corner topology (3 primitives per node)
                # and both neighbor edge dicts present.
                if len(startnode_primitives) != 3 or len(endnode_primitives) != 3:
                    continue
                if not _keys_ok(startnode_primitives.difference(edge_primitives), 'end_', loop_node_idx[0]) \
                   or not _keys_ok(endnode_primitives.difference(edge_primitives), 'start_', loop_node_idx[-1]):
                    continue
            ...
            if len(startnode_primitives.difference(edge_primitives)) == 0 and len(edge_primitives.difference(edge_primitives)) == 0:
                ...
                if len(final_edge) == 0:
                    # No connected path between the corner nodes was found in the
                    # cut-result edge graph (degenerate/duplicate boundary on a noisy
                    # mesh region) — skip this loop entry instead of indexing into [].
                    print(f"[GUARD] get_final_edge returned no edges for loop {loop_index}, entry skipped")
                    continue
    ...
    all_wires = []
    _valid_loops, _valid_mesh_loops, _valid_edge_status, _valid_edge_maps = [], [], [], []
    for _li, loop in enumerate(out_loops):
        c_wire = BRepBuilderAPI_MakeWire()
        for edges in loop:
            for e in edges:
                e = e.Oriented(TopAbs_FORWARD)
                c_wire.Add(e)
        # A loop whose edges were partially skipped cannot form a connected wire;
        # drop it (and keep the parallel lists aligned) instead of raising NotDone.
        if not c_wire.IsDone():
            print(f"[GUARD] wire NOT done for loop {_li} (edges={sum(len(e) for e in loop)}), dropped")
            continue
        all_wires.append(c_wire.Wire())
        _valid_loops.append(loop)
        _valid_mesh_loops.append(out_mesh_loops[_li])
        _valid_edge_status.append(out_loops_edge_status[_li])
        _valid_edge_maps.append(out_edge_maps[_li])
    out_loops = _valid_loops
    out_mesh_loops = _valid_mesh_loops
    out_loops_edge_status = _valid_edge_status
    out_edge_maps = _valid_edge_maps
    all_wires_length = [calculate_wire_length(ww) for ww in all_wires]
    ...
    for i in range(len(c_all_wires)):
        ...
        if len(candidate_faces) == 0:
            # The wire/mesh-flag intersection test found no matching face for this
            # loop (degenerate split on a noisy/placeholder region) — keep the shape
            # as split by previous iterations instead of crashing on candidate_faces[0].
            print(f"[GUARD] get_loop_face: no candidate face for wire {i}, keeping previous c_shape")
            continue
        elif len(candidate_faces) > 1 and not BRep_Tool_Surface(candidate_faces[0]).IsKind(Geom_ToroidalSurface.__name__):
            try:
                sewing = BRepBuilderAPI_Sewing(1e-5)
                for ff in candidate_faces:
                    sewing.Add(ff)
