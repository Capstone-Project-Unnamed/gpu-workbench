...
while not os.path.exists(os.path.join(_repo_root, "freecad_stub.py")):
...
from neus.fit_surfaces.fitting_one_surface import process_one_surface
from neus.fit_surfaces.fitting_utils import project_to_plane

sys.path.append(os.path.join(_repo_root, "pyransac", "cmake-build-release"))
import fitpoints
...
from neus.intersectiontrace import intersection_between_face_shapes_track
...
def save_step_file(faces, output_path):
    from freecad_stub import _OCCShape as _FCADShape
    from OCC.Core.TopExp import TopExp_Explorer
    ...
    for face in faces:
        if face is None:
            continue
        occ_face = face._shape if isinstance(face, _FCADShape) else face
        if occ_face is None:
            continue
        try:
            exp = TopExp_Explorer(occ_face, TopAbs_FACE)
            if exp.More():
                while exp.More():
                    collected.append(topods.Face(exp.Current()))
                    exp.Next()
            else:
                collected.append(occ_face)
        except Exception as e:
            print(f"Warning: could not collect face: {e}")

    print(f"save_step_file: collected {len(collected)}/{len(faces)} faces")
    if len(collected) == 0:
        print("Warning: no valid faces to write step file")
        return

    # Merely putting independently-built faces into a shell does NOT share their
    # coincident edges/vertices (each face keeps its own separate topology, so the
    # result is N disconnected patches, not a watertight solid). Sew them together
    # within a tolerance so adjacent faces actually share edges/vertices, then try
    # to close the result into a solid.
    ...
    while exp.More():
        added += 1
        exp.Next()
    print(f"save_step_file: sewed shape contains {added} faces")

    out_shape = sewn_shape
    n_shells = 0
    ...
    if n_shells == 1:
        try:
            ...
            if solid_maker.IsDone():
                candidate_solid = solid_maker.Solid()
                # Only keep the solid if it's actually a valid closed body — an
                # invalid forced solid can lose faces on the STEP round-trip, which
                # is worse than exporting the (still properly sewn) open shell.
                if BRepCheck_Analyzer(candidate_solid).IsValid():
                    out_shape = candidate_solid
                    print("save_step_file: closed sewn shell into a valid solid")
                else:
                    print("save_step_file: solid conversion produced an invalid body, keeping sewn shell instead")
        except Exception as e:
            print(f"save_step_file: could not build solid from sewn shell ({e}), keeping sewn shape")

    analyzer = BRepCheck_Analyzer(out_shape)
    print(f"save_step_file: BRep valid = {analyzer.IsValid()}, shells = {n_shells}")

    if added > 0:
        write_step_file(out_shape, output_path, application_protocol="AP214IS")
        print(f"Saved: {output_path}")
    else:
        print("Warning: no valid faces to write step file")


def _merge_near_parallel_adjacent_planes(trimm_all_labels, new_trimm, ransac_params, tol=0.01):
    """Segmentation sometimes over-splits one physical planar face into two (or more)
    mesh-adjacent Plane primitives whose normals differ only slightly (near-coplanar,
    small dihedral angle). Left alone, the intersection tracer tries to find a boundary
    edge between them, but since they are almost parallel the true intersection line of
    the two infinite planes lies far outside the local mesh patch, so the search always
    comes back empty and face assembly fails for the whole shape. Detect adjacent plane
    pairs whose normals are within `tol` of parallel (1-|cos(theta)| < tol) and fold them
    into a single averaged plane.

    Returns (new_labels, merged_params, did_merge). `new_labels` keeps the same label
    *values* as `trimm_all_labels` (no compaction) — the caller is responsible for
    compacting to a dense 0..k-1 range afterwards, since several downstream loops assume
    `range(0, len(set(labels)))` covers exactly the surviving primitive indices.
    """
    n = len(ransac_params)
    face_adj = new_trimm.face_adjacency
    labels_at_adj = trimm_all_labels[face_adj]
    adj_pairs = set()
    for a, b in labels_at_adj:
        if a != b:
            adj_pairs.add((int(min(a, b)), int(max(a, b))))
    ...
    for a, b in adj_pairs:
        # trimm_all_labels may contain label values with no corresponding fitted
        # primitive (e.g. a label whose RANSAC fit was dropped/failed) — skip those
        # instead of indexing out of range.
        if not (0 <= a < n) or not (0 <= b < n):
            continue
        pa, pb = ransac_params[a], ransac_params[b]
        if pa is None or pb is None:
            continue
        if pa.getType() != 'Plane' or pb.getType() != 'Plane':
            continue
        na = pa.normal / (np.linalg.norm(pa.normal) + 1e-12)
        nb = pb.normal / (np.linalg.norm(pb.normal) + 1e-12)
        if abs(1 - abs(np.dot(na, nb))) < tol:
            union(a, b)
            merged_any = True

    if not merged_any:
        return trimm_all_labels, ransac_params, False

    roots = np.array([find(i) for i in range(n)])
    new_labels = trimm_all_labels.copy()
    merged_params = list(ransac_params)
    for root in set(roots.tolist()):
        members = np.where(roots == root)[0]
        if len(members) <= 1:
            continue
        face_counts = np.array([np.sum(trimm_all_labels == m) for m in members], dtype=float)
        if face_counts.sum() == 0:
            continue
        normals = np.array([ransac_params[m].normal / (np.linalg.norm(ransac_params[m].normal) + 1e-12) for m in members])
        signs = np.sign(normals @ normals[0])
        signs[signs == 0] = 1
        avg_normal = (normals * signs[:, None] * face_counts[:, None]).sum(axis=0)
        avg_normal /= (np.linalg.norm(avg_normal) + 1e-12)
        positions = np.array([ransac_params[m].pos for m in members])
        avg_pos = (positions * face_counts[:, None]).sum(axis=0) / face_counts.sum()
        merged_plane = Plane(avg_pos, avg_normal)
        for m in members:
            new_labels[trimm_all_labels == m] = root
            merged_params[m] = merged_plane
        print(f"[MERGE] near-parallel adjacent planes {members.tolist()} -> single plane (root label {root})")

    return new_labels, merged_params, True
...
def fit_and_intersection(count, trimm_face_centers, trimm_all_labels, new_trimm, components_labels, new_trimm_face_labels, bind_parallel_comps, ransac_params=None, cfg=None,  correct=True):
    if ransac_params is not None:
        # Pre-existing latent inconsistency: the per-face label array can contain a
        # label value with no corresponding fitted primitive (e.g. a tiny orphan
        # region dropped during an earlier filtering step). Clamp such faces to the
        # nearest valid primitive index instead of letting them propagate into an
        # out-of-range crash later.
        _n_params = len(ransac_params)
        _bad = (trimm_all_labels < 0) | (trimm_all_labels >= _n_params)
        if np.any(_bad):
            print(f"[GUARD] {int(np.sum(_bad))} mesh faces have a label with no fitted primitive; clamping to a valid index")
            trimm_all_labels = np.clip(trimm_all_labels, 0, _n_params - 1)
            new_trimm_face_labels = trimm_all_labels

        _merged_labels, _merged_params, _did_merge = _merge_near_parallel_adjacent_planes(
            trimm_all_labels, new_trimm, ransac_params)
        if _did_merge:
            # Compact label values to a dense 0..k-1 range: several downstream loops
            # (e.g. renew_newton_scales) assume range(0, len(set(labels))) exactly
            # covers the surviving primitive indices, so gaps left by the merge above
            # would silently misalign primitive geometry with mesh regions otherwise.
            _uniq, _inv = np.unique(_merged_labels, return_inverse=True)
            trimm_all_labels = _inv.reshape(_merged_labels.shape).astype(trimm_all_labels.dtype)
            new_trimm_face_labels = trimm_all_labels
            ransac_params = [_merged_params[int(old)] for old in _uniq]
            count = len(_uniq) + 1
            print(f"[MERGE] compacted {len(_merged_params)} -> {len(_uniq)} primitives after merging near-parallel adjacent planes")
    ...
    if faces is None and o_faces is None:
        print("Warning: both faces and o_faces are None, returning empty list.")
        return []
    valid_path  = cfg.output_path + "/temp_valid_new_correct" + os.path.basename(cfg.config_dir)
    result = o_faces if o_faces is not None else faces
    save_cache_dill([result, tshapes], valid_path)
    return result
