...
def filter_points(face_idx, face_shape_distances, k=10):
    n_shapes = np.array(face_shape_distances).shape[1] if hasattr(face_shape_distances, '__len__') else 1
    take = min(2, n_shapes)
    min_indices = np.argsort(face_shape_distances, axis=1)[:, :take]
    # pad to 2 columns if only 1 shape
    if take < 2:
        min_indices = np.pad(min_indices, ((0,0),(0,2-take)), constant_values=0)
