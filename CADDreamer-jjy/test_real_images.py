...
import blenderproc.api.math as _bproc_math
import blenderproc.api.camera as _bproc_camera
bproc.math = _bproc_math
bproc.camera = _bproc_camera
...
def process_final_connnected_components_objs(mesh, face_components, intial_objs,  label_components):
    ...
    for assign_comp_idx, assign_comp in enumerate(assign_label_of_components):
        ...
        # Isolated fragment with no main-component neighbor: keep its own label, skip reassign.
        if len(potential_neighrbos) == 0:
            continue
...
def export_labeled_mesh(mesh, face_labels, out_path):
    """Debug export: color each face by its integer label and write a .ply. label -1 = unlabeled (gray)."""
    face_labels = np.asarray(face_labels).astype(np.int64)
    cmap = matplotlib.colormaps.get_cmap('tab20')
    unique_labels = [l for l in np.unique(face_labels) if l != -1]
    face_colors = np.full((len(mesh.faces), 4), [128, 128, 128, 255], dtype=np.uint8)
    for i, lbl in enumerate(unique_labels):
        rgba = (np.array(cmap(i % 20)) * 255).astype(np.uint8)
        face_colors[face_labels == lbl] = rgba
    vis_mesh = mesh.copy()
    vis_mesh.visual.face_colors = face_colors
    vis_mesh.export(out_path)
...
if __name__=='__main__':
    ...
    vis_dir = os.path.join(OUTPUT_PATH, "vis_" + os.path.basename(cfg.config_dir))
    ...
    if not os.path.exists(temp_path) or cfg.review:
        ...
        with time_block("Third step: build graph between partial instances, and execute graph cut."):
            ...
            export_labeled_mesh(real_mesh, new_partial_components_sure_labeles - 1, os.path.join(vis_dir, "6_primitive_instances_raw.ply"))
        ...
        with time_block("Fourth step: you konw the primitive stitching rely on a good segmentation boundary, so we need to do label smooth.  "):
            ...
            graphcut_labels = np.full(len(real_mesh.faces), -1, dtype=np.int64)
            for comp_idx, comp_faces in enumerate(face_components):
                graphcut_labels[comp_faces] = comp_idx
            export_labeled_mesh(real_mesh, graphcut_labels, os.path.join(vis_dir, "7_graphcut.ply"))
    ...
    if not os.path.exists(temp_path) or cfg.review:
        with time_block("Fifth step: ransac fit of each instance's primitives. "):
            ...
            # Patches with too few faces produce degenerate primitive fits (near-random
            # radius/axis) that can later "win" the nearest-primitive assignment for
            # unrelated faces elsewhere on the mesh and blow up the final BRep shape.
            # 80 gives pyransac's own min_points=30 requirement comfortable margin --
            # matches remove_abuntant_objs's min_component_size (get_segmentation_result.py).
            MIN_PATCH_FACES = 80
            for idx in range(len(patch_comps)):
                c_patch, c_patch_face_idx, c_patch_labels = patch_comps[idx]
                if len(c_patch_face_idx) < MIN_PATCH_FACES:
                    continue
        ...
        with time_block("Ninth step: save the final segmentation results. "):
            ...
            assert new_labels.max()+1 == len(compact_cad_objs_label)
            ...
            export_labeled_mesh(new_trimm, new_trimm_face_labels, os.path.join(vis_dir, "8_final_segmentation.ply"))
