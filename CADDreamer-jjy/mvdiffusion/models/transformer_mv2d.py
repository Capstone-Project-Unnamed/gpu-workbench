    ...
    def __call__(
        ...
        if multiview_attention:
            if not sparse_mv_attention:
                ...
                key_front = rearrange(key, "(b t) d c -> b t d c", t=num_views)[:, 0, :, :].repeat_interleave(num_views, dim=0)
                value_front = rearrange(value, "(b t) d c -> b t d c", t=num_views)[:, 0, :, :].repeat_interleave(num_views, dim=0)
                key = torch.cat([key_front, key], dim=1)
                value = torch.cat([value_front, value], dim=1)
